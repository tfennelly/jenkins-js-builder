...
describe("adjunctexternal test", function () {
    ...
    it("- test NPM org package name", function () {
        ...
        var indexOfPart = fileContents.indexOf("jsModules.exportModule('jenkins-cd-js-modules', 'jenkins-cd-js-modules@0.0.8', exportModule);");
        expect(indexOfPart !== -1).toBe(true);
        // And that the 'any' version is exported.
        indexOfPart = fileContents.indexOf("jsModules.exportModule('jenkins-cd-js-modules', 'jenkins-cd-js-modules@any', exportModule);");
        expect(indexOfPart !== -1).toBe(true);
    });
});