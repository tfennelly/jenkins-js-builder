...
describe("adjunctexternal test", function () {
    ...
    it("- test NPM org package name", function () {
        ...
        var indexOfPart = fileContents.indexOf("___$$$___doExport('jenkins-cd-js-modules', 'jenkins-cd-js-modules@0.0.8', entryModule);");
        expect(indexOfPart !== -1).toBe(true);
        // And that the 'any' version is exported.
        indexOfPart = fileContents.indexOf("___$$$___doExport('jenkins-cd-js-modules', 'jenkins-cd-js-modules@any', entryModule);");
        expect(indexOfPart !== -1).toBe(true);
    });
});